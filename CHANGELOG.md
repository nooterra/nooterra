...
- Identity transparency log surface: append-only identity lifecycle events (`create|rotate|revoke|capability-claim-change`) with deterministic Merkle checkpoint/proof APIs (`GET /v1/public/identity-log/entries|checkpoint|proof`) and CLI verification (`nooterra identity log verify`) that fail closed on mismatch/equivocation.
