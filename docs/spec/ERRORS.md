...
## NS3 evidence-binding reason codes (registry-linked)

The canonical machine-readable matrix is `docs/kernel-compatible/ns3-evidence-binding-coverage-matrix.json`.
All reason codes referenced by that matrix are listed below and are also required to exist in `docs/spec/x402-error-codes.v1.txt`.
