...
function usage() {
  writeStderr(
    [
      "usage:",
      "  nooterra-trust init --out <dir> [--format json|text] [--force] [--with-time-authority]",
      "  nooterra-trust init --mode remote-only --out <dir> --governance-root-key-id <id> [--time-authority-key-id <id>] (--signer-url <url> [--signer-auth bearer --signer-token-env <ENV>|--signer-token-file <path>] [--signer-header \"X-Foo: bar\"] | --signer-command <cmd> --signer-args-json <json>) [--format json|text] [--force]",
      ""
    ].join("\n")
  );
  process.exit(2);
