...
function usage() {
  writeStderr(
    [
      "usage:",
      "  nooterra-release verify --dir <release-assets-dir> [--trust-file <path>] [--check-installed <nooterra-verify>] [--format human|json] [--explain]",
      "  nooterra-release verify --base-url <url> [--trust-file <path>] [--check-installed <nooterra-verify>] [--format human|json] [--explain] [--offline]",
      "  nooterra-release verify --github-repo <owner/repo> --tag <tag> [--trust-file <path>] [--check-installed <nooterra-verify>] [--format human|json] [--explain] [--offline]"
    ].join("\n") + "\n"
  );
  process.exit(2);
}
...
function httpGetBuffer(url, { timeoutMs, maxBytes }) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(
      ...
      {
        method: "GET",
        headers: { "user-agent": "nooterra-release" }
      },
      (res) => {
...
function formatExplain({ source, trustFile, result }) {
  const lines = [];
  lines.push("nooterra-release explain v1");
...
async function main() {
  ...
  if (args.dir) {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nooterra-release-"));
    if (args.baseUrl) {
  ...
  if (args.format === "json") {
    ...
    const lines = [];
    lines.push("nooterra-release verify");
