...
function usage() {
  const text = [
    "usage:",
    "  nooterra-verify --version",
    "  nooterra-verify --about [--format human|json]",
    "  nooterra-verify <artifact.json> [artifact2.json ...]",
    "  nooterra-verify --reconcile <month-proof-bundle-dir>",
    "  nooterra-verify [--strict] [--fail-on-warnings] [--explain] [--format human|json|sarif] [--report-json <path>] [--hash-concurrency <n>] --job-proof <JobProofBundle.v1.zip|dir>",
    "  nooterra-verify [--strict] [--fail-on-warnings] [--explain] [--format human|json|sarif] [--report-json <path>] [--hash-concurrency <n>] --month-proof <MonthProofBundle.v1.zip|dir>",
    "  nooterra-verify [--strict] [--fail-on-warnings] [--explain] [--format human|json|sarif] [--report-json <path>] [--hash-concurrency <n>] --finance-pack <FinancePackBundle.v1.zip|dir>",
    "  nooterra-verify [--strict] [--fail-on-warnings] [--explain] [--format human|json|sarif] [--report-json <path>] [--hash-concurrency <n>] --invoice-bundle <InvoiceBundle.v1.zip|dir> [--settlement-decision <SettlementDecisionReport.v1.json> --trusted-buyer-keys <keys.json|json>]",
    "  nooterra-verify [--strict] [--fail-on-warnings] [--explain] [--format human|json|sarif] [--report-json <path>] [--hash-concurrency <n>] --close-pack <ClosePack.v1.zip|dir>"
  ].join("\n");
...
function formatCliOutput({ kind, input, resolved, dir, strict, failOnWarnings, result, toolVersion, toolCommit }) {
  ...
  return {
    ...
    tool: { name: "nooterra-verify", version: toolVersion ?? null, commit: toolCommit ?? null },
...
async function writeExplain({ kind, input, resolved, dir, strict, failOnWarnings, format, hashConcurrency, result, cliOut }) {
  const lines = [];
  lines.push("nooterra-verify explain v1");
  ...
  lines.push(`trust.governanceRoots.count=${tryParseKeyCount(process.env.NOOTERRA_TRUSTED_GOVERNANCE_ROOT_KEYS_JSON ?? "")}`);
  lines.push(`trust.timeAuthorities.count=${tryParseKeyCount(process.env.NOOTERRA_TRUSTED_TIME_AUTHORITY_KEYS_JSON ?? "")}`);
  lines.push(`trust.pricingSigners.count=${tryParseKeyCount(process.env.NOOTERRA_TRUSTED_PRICING_SIGNER_KEYS_JSON ?? "")}`);
    ...
    runs: [
      {
        tool: {
          driver: {
            name: cliOut?.tool?.name ?? "nooterra-verify",
...
async function main() {
  ...
  if (about) {
    ...
    else await writeStdout(`nooterra-verify ${toolVersion ?? "unknown"} (${toolCommit ?? "commit-unknown"})\n`);
    return 0;
  }
  if (!args.length) {
    usage();
  ...
  if (args[0] === "--invoice-bundle") {
    ...
    if (settlementDecisionPath) {
      ...
      const rawKeys = trustedBuyerKeys ?? process.env.NOOTERRA_TRUSTED_SETTLEMENT_DECISION_SIGNER_KEYS_JSON ?? "";
      const keysText = String(rawKeys ?? "").trim();
      if (!keysText) {
        const result = { ok: false, error: "settlement decision trusted buyer keys missing", env: "NOOTERRA_TRUSTED_SETTLEMENT_DECISION_SIGNER_KEYS_JSON", warnings: [] };
        if (reportJsonPath) await fs.writeFile(reportJsonPath, canonicalJsonStringify(result) + "\n", "utf8");
        if (format === "json" || format === "sarif") {
          const out = formatCliOutput({ kind: "settlement-decision", input: decisionInput, resolved: decisionResolved, dir, strict, failOnWarnings, result, toolVersion, toolCommit });
          if (explain) await writeExplain({ kind: "settlement-decision", input: decisionInput, resolved: decisionResolved, dir, strict, failOnWarnings, format, hashConcurrency, result, cliOut: out });
      ...
      const tenantId = (() => {
        try {
          const j = JSON.parse(fsSync.readFileSync(path.join(dir, "nooterra.json"), "utf8"));
