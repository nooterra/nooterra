...
export function usage() {
  return [
    "Usage:",
    "  create-nooterra-paid-tool [directory] [--force] [--provider-id <id>] [--from-http <baseUrl>] [--from-openapi <specPath>]",
    "",
    "Options:",
...
export function parseArgs(argv) {
  ...
  out.directory = positional[0] ?? "nooterra-paid-tool";
...
function buildTemplate({ providerId }) {
  ...
  const envExample = [
    ...
    "NOOTERRA_PRICE_AMOUNT_CENTS=500",
    "NOOTERRA_PRICE_CURRENCY=USD",
    "NOOTERRA_PAYMENT_ADDRESS=mock:payee",
    "NOOTERRA_PAYMENT_NETWORK=mocknet",
...
const providerId = typeof process.env.NOOTERRA_PROVIDER_ID === "string" && process.env.NOOTERRA_PROVIDER_ID.trim() !== ""
  ? process.env.NOOTERRA_PROVIDER_ID.trim()
  : "${providerId}";
const amountCents = Number(process.env.NOOTERRA_PRICE_AMOUNT_CENTS ?? 500);
if (!Number.isSafeInteger(amountCents) || amountCents <= 0) throw new Error("NOOTERRA_PRICE_AMOUNT_CENTS must be positive");
const currency = typeof process.env.NOOTERRA_PRICE_CURRENCY === "string" && process.env.NOOTERRA_PRICE_CURRENCY.trim() !== ""
...
const paidHandler = createNooterraPaidNodeHttpHandler({
  ...
  paymentAddress: process.env.NOOTERRA_PAYMENT_ADDRESS ?? "mock:payee",
  paymentNetwork: process.env.NOOTERRA_PAYMENT_NETWORK ?? "mocknet",
  priceFor: ({ req, url }) => ({
    amountCents,
    currency,
    providerId,
  ...
  nooterraPay: {
    keysetUrl: process.env.NOOTERRA_PAY_KEYSET_URL ?? "http://127.0.0.1:3000/.well-known/nooterra-keys.json"
  },
  execute: async ({ url }) => ({
    body: {
      ok: true,
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(JSON.stringify({ ok: true, service: "nooterra-paid-tool", port: PORT, providerId }));
});
`;

  const readme = `# Nooterra Paid Tool Template

This project was generated by \`create-nooterra-paid-tool\`.
...
- \`GET /tool/search?q=...\` returns \`402\` until a valid \`Authorization: NooterraPay <token>\` is provided.
- On paid requests, the server verifies NooterraPay offline and returns provider signature headers:
  - \`x-nooterra-provider-key-id\`
...
If \`@nooterra/provider-kit\` is not yet published to npm, replace it with your internal tarball or git source.
`;
...
function buildBridgeTemplate({ providerId, upstreamBaseUrl, manifest }) {
  const packageJson = {
    name: "nooterra-paid-tool-bridge",
    version: "0.0.0",
    private: true,
    type: "module",
    engines: {
...
const providerId = String(process.env.NOOTERRA_PROVIDER_ID ?? manifest?.providerId ?? "${providerId}").trim();
if (!providerId) throw new Error("NOOTERRA_PROVIDER_ID is required");
...
const paidHandler = createNooterraPaidNodeHttpHandler({
  ...
  priceFor: ({ url }) => {
    ...
    const amountCents = Number.isSafeInteger(Number(pricing.amountCents)) ? Number(pricing.amountCents) : Number(process.env.NOOTERRA_PRICE_AMOUNT_CENTS ?? 500);
    const currency =
      typeof pricing.currency === "string" && pricing.currency.trim() !== ""
        ? pricing.currency.trim().toUpperCase()
        : String(process.env.NOOTERRA_PRICE_CURRENCY ?? "USD").trim().toUpperCase();
    return {
      amountCents,
      currency,
      providerId,
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(JSON.stringify({ ok: true, service: "nooterra-paid-tool-bridge", port: PORT, providerId, tools: toolByPaidPath.size }));
});
`;
...
import { fetchWithNooterraAutopay } from "nooterra-api-sdk";
...
async function callTool(toolConfig, args) {
  ...
  const res = await fetchWithNooterraAutopay(url, init);
    ...
    if (method === "initialize") {
      send({
        ...
        result: {
          ...
          serverInfo: { name: "nooterra-paid-tool-bridge", version: "0.0.0" },
          capabilities: { tools: {} }
        }
      });
      continue;
...
`;
  ...
  const readme = `# Nooterra Paid Tool Bridge Template

This template was generated from an upstream API and includes:

- A paid HTTP bridge server (\`server.mjs\`) using \`@nooterra/provider-kit\`
...
export function scaffoldCreateNooterraPaidTool({
  directory,
  force = false,
  providerId = "prov_paid_tool_demo",
  fromHttp = null,
...
export function runCreateNooterraPaidToolCli({ argv = process.argv.slice(2), cwd = process.cwd(), stdout = process.stdout, stderr = process.stderr } = {}) {
  let args;
  try {
    args = parseArgs(argv);
  } catch (err) {
  ...
  const result = scaffoldCreateNooterraPaidTool({
