...
import { bootstrapLocalGateEnv, bootstrapLocalPgEnv } from "./local-bootstrap.mjs";

const SCHEMA_VERSION = "NooterraVerifiedGateReport.v1";
...
function usage() {
  return [
    "usage: node scripts/ci/run-nooterra-verified-gate.mjs [options]",
    "",
    "options:",
    "  --level <core|collaboration|guardrails>  Verification level (default: guardrails)",
    "  --out <file>                              Report path (default: artifacts/gates/nooterra-verified-gate.json)",
    ...
    "  --include-pg                             Include PG durability check (requires DATABASE_URL or --bootstrap-local)",
    "  --help                                    Show help"
  ].join("\n");
}
...
export function parseArgs(argv, env = process.env, cwd = process.cwd()) {
  const out = {
    level: "guardrails",
    out: path.resolve(cwd, "artifacts/gates/nooterra-verified-gate.json"),
    help: false,
    ...
    includePg: parseBooleanFlag(env?.NOOTERRA_VERIFIED_INCLUDE_PG ?? "0", { fieldName: "NOOTERRA_VERIFIED_INCLUDE_PG" }),
  ...
  if (out.includePg && !out.databaseUrl && !out.bootstrapLocal) {
    throw new Error("--include-pg requires DATABASE_URL (or --bootstrap-local for docker-backed local PG bootstrap)");
  }
  return out;
}
...
function checksForLevel(level, { includePg = false } = {}) {
  const core = [
    ...
    { id: "mcp_probe_about", command: "npm", args: ["run", "-s", "mcp:probe", "--", "--call", "nooterra.about", "{}"] },
  ...
  const collaboration = [
    ...
    { id: "e2e_state_checkpoint_routes", command: "node", args: ["--test", "test/api-e2e-state-checkpoints.test.js"] },
    ...
    { id: "e2e_authority_grant_required", command: "node", args: ["--test", "test/api-e2e-authority-grant-required.test.js"] },
    {
      ...
      command: "node",
      args: ["scripts/demo/run-openclaw-substrate-demo.mjs", "--out", "artifacts/demo/nooterra-verified-openclaw-substrate-demo.json"],
      demoReportPath: "artifacts/demo/nooterra-verified-openclaw-substrate-demo.json",
  ...
  const pg = includePg
    ? [
        ...
        { id: "pg_state_checkpoint_durability", command: "node", args: ["--test", "test/pg-state-checkpoints.test.js"] },
        { id: "pg_work_order_metering_durability", command: "node", args: ["--test", "test/pg-work-order-metering-durability.test.js"] }
      ]
    : [];
...
export async function runNooterraVerifiedGate(args, options = {}) {
  const runCheckFn = typeof options.runCheckFn === "function" ? options.runCheckFn : runCheck;
  const bootstrapApiFn = typeof options.bootstrapFn === "function" ? options.bootstrapFn : bootstrapLocalGateEnv;
  const bootstrapPgFn = typeof options.bootstrapPgFn === "function" ? options.bootstrapPgFn : bootstrapLocalPgEnv;
  const checks = checksForLevel(args.level, { includePg: Boolean(args.includePg) });
  const includesPgChecks = checks.some((check) => String(check?.id ?? "").startsWith("pg_"));
  const bootstrapApi = await bootstrapApiFn({
  ...
  const bootstrapPg = await bootstrapPgFn({
    enabled: includesPgChecks,
    databaseUrl: String(args.databaseUrl ?? "").trim() || null,
    logger: (line) => process.stderr.write(`[bootstrap-pg] ${line}\n`)
  });
  ...
  try {
    ...
    for (const check of checks) {
      const row = await Promise.resolve(
        runCheckFn({
          ...
          env: {
            ...(check.env ?? {}),
            ...(bootstrapApi.envPatch ?? {}),
            ...(bootstrapPg.envPatch ?? {})
          }
        })
      );
    ...
    await bootstrapPg.cleanup?.();
    await bootstrapApi.cleanup?.();
  }
...
async function main() {
  ...
  const { report } = await runNooterraVerifiedGate(args);
