...
async function enforceLaunchPacketCollaborationBinding({ artifacts, args, cwd }) {
  ...
  if (productionArtifact && productionArtifact.readOk === true && productionArtifact.parseOk === true) {
    ...
    if (productionLoaded.readOk && productionLoaded.parseOk) {
      ...
      const sessionStreamConformanceCheck =
        checks.find((row) => normalizeOptionalString(row?.id) === PRODUCTION_SESSION_STREAM_CONFORMANCE_CHECK_ID) ?? null;
      if (!sessionStreamConformanceCheck) {
        markArtifactFailed(
          launchArtifact,
          "production_gate_session_stream_conformance_check_missing",
          "production cutover gate missing session stream conformance verification check"
        );
        result.failureCodes.push("production_gate_session_stream_conformance_check_missing");
      } else if (normalizeOptionalString(sessionStreamConformanceCheck?.status) !== "passed") {
        markArtifactFailed(
          launchArtifact,
          "production_gate_session_stream_conformance_check_not_passed",
