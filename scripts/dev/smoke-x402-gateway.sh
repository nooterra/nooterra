...
export NOOTERRA_ENV_FILE="/dev/null"

echo "[smoke] starting Nooterra API on :3000"
PROXY_OPS_TOKEN=tok_ops PORT=3000 node src/api/server.js >/tmp/nooterra_api_smoke.log 2>&1 &
API_PID="$!"
...
for i in $(seq 1 60); do
  ...
  if [[ "$i" == "60" ]]; then
    ...
    tail -n 200 /tmp/nooterra_api_smoke.log >&2 || true
    exit 1
  fi
done
...
if [[ -z "${NOOTERRA_API_KEY:-}" ]]; then
  echo "[smoke] NOOTERRA_API_KEY not set after new-sdk-key.sh" >&2
  exit 1
fi

echo "[smoke] starting mock upstream on :9402"
PORT=9402 node services/x402-gateway/examples/upstream-mock.js >/tmp/nooterra_x402_upstream_smoke.log 2>&1 &
UPSTREAM_PID="$!"
...
for i in $(seq 1 30); do
  ...
  if [[ "$i" == "30" ]]; then
    ...
    tail -n 200 /tmp/nooterra_x402_upstream_smoke.log >&2 || true
    exit 1
  fi
done
...
PORT=8402 \
node services/x402-gateway/src/server.js >/tmp/nooterra_x402_gateway_smoke.log 2>&1 &
GATEWAY_PID="$!"
...
for i in $(seq 1 30); do
  ...
  if [[ "$i" == "30" ]]; then
    ...
    tail -n 200 /tmp/nooterra_x402_gateway_smoke.log >&2 || true
    exit 1
  fi
done
...
GATE_ID="$(
  awk 'BEGIN{IGNORECASE=1} $1 ~ /^x-nooterra-gate-id:$/ {print $2}' "${FIRST_HEADERS}" | tr -d '\r' | head -n 1
)"
if [[ -z "${GATE_ID:-}" ]]; then
  echo "[smoke] missing x-nooterra-gate-id header" >&2
...
curl -fsS "http://127.0.0.1:3000/x402/gate/${GATE_ID}" \
  ...
  -H "x-nooterra-protocol: 1.0" | jq '{gateStatus:.gate.status, settlementStatus:.settlement.status, holdback:.gate.holdback}'
