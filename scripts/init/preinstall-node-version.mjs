...
const allowUnsupportedNode = String(process.env.NOOTERRA_ALLOW_UNSUPPORTED_NODE ?? "").trim() === "1";

if (!allowUnsupportedNode && currentNodeMajor !== REQUIRED_NODE_MAJOR) {
  process.stderr.write(
    [
      `[nooterra] error: Node.js ${REQUIRED_NODE_MAJOR}.x is required for deterministic local behavior.`,
      ...
      "  NOOTERRA_ALLOW_UNSUPPORTED_NODE=1 npm ci"
    ].join("\n") + "\n"
  );
  process.exit(1);
}
