...
function usage() {
  return [
    ...
    "  --website-base-url <url>           Website base URL for same-origin onboarding checks",
...
export function parseArgs(argv, env = process.env, cwd = process.cwd()) {
  ...
  out.baseUrl = normalizeOptionalString(out.baseUrl)?.replace(/\/+$/, "") ?? null;
  out.websiteBaseUrl = normalizeOptionalString(out.websiteBaseUrl)?.replace(/\/+$/, "") ?? null;
