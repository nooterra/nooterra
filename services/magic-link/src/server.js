...
function tenantHostedApprovalHistoryPath({ tenantId }) {
  return path.join(dataDir, "tenants", tenantId, "onboarding_hosted_approvals.json");
}
...
function defaultTenantHostedApprovalHistory({ tenantId }) {
  return {
...
function normalizeHostedApprovalAttemptRow(input) {
  ...
  const actorAgentId = typeof input.actorAgentId === "string" && input.actorAgentId.trim() ? safeTruncate(input.actorAgentId.trim(), { max: 140 }) : null;
  const principalId = typeof input.principalId === "string" && input.principalId.trim() ? safeTruncate(input.principalId.trim(), { max: 320 }) : null;
  const error = isPlainObject(input.error)
    ...
    : null;
  return {
    schemaVersion: "MagicLinkHostedApprovalAttempt.v1",
    attemptId,
    createdAt,
    completedAt,
    status,
...
async function loadTenantHostedApprovalHistoryBestEffort({ tenantId }) {
  const fp = tenantHostedApprovalHistoryPath({ tenantId });
  try {
    const raw = JSON.parse(await fs.readFile(fp, "utf8"));
    if (!isPlainObject(raw)) return defaultTenantHostedApprovalHistory({ tenantId });
    const rows = Array.isArray(raw.attempts) ? raw.attempts : [];
    const attempts = rows.map((row) => normalizeHostedApprovalAttemptRow(row)).filter(Boolean).slice(-100);
    const updatedAt = typeof raw.updatedAt === "string" && Number.isFinite(Date.parse(raw.updatedAt)) ? raw.updatedAt : null;
    return {
      schemaVersion: "MagicLinkHostedApprovalHistory.v1",
      tenantId,
    ...
    return defaultTenantHostedApprovalHistory({ tenantId });
  }
}
...
async function saveTenantHostedApprovalHistory({ tenantId, history }) {
  const fp = tenantHostedApprovalHistoryPath({ tenantId });
  await fs.mkdir(path.dirname(fp), { recursive: true });
  await fs.writeFile(fp, JSON.stringify(history, null, 2) + "\n", "utf8");
}
...
async function appendTenantHostedApprovalAttempt({ tenantId, attempt }) {
  const history = await loadTenantHostedApprovalHistoryBestEffort({ tenantId });
  const normalized = normalizeHostedApprovalAttemptRow(attempt);
  if (!normalized) return history;
  const attempts = [...history.attempts, normalized].slice(-100);
  const next = {
    schemaVersion: "MagicLinkHostedApprovalHistory.v1",
...
async function handleTenantSeedHostedApproval(req, res, tenantId) {
  ...
  const attemptId = requestIdPrefix;
  const startedAt = nowIso();
  ...
  if (!bootstrap.ok) {
    await appendTenantHostedApprovalAttempt({
      tenantId,
      attempt: {
        attemptId,
        createdAt: startedAt,
        ...
        actorAgentId,
        error: {
          code: bootstrap.code ?? "RUNTIME_BOOTSTRAP_FAILED",
          message: bootstrap.message ?? "runtime bootstrap failed"
        }
      }
    });
  ...
  if (!createdIntent.ok) {
    await appendTenantHostedApprovalAttempt({
      ...
      attempt: {
        ...
        actionIntentId,
        error: {
          code: createdIntent.code ?? "NOOTERRA_API_CALL_FAILED",
          message: createdIntent.message ?? "action intent creation failed"
        }
      }
    });
  ...
  if (!approvalRequested.ok) {
    await appendTenantHostedApprovalAttempt({
      ...
      attempt: {
        ...
        error: {
          code: approvalRequested.code ?? "NOOTERRA_API_CALL_FAILED",
          message: approvalRequested.message ?? "approval request creation failed"
        }
      }
    });
  ...
  const history = await appendTenantHostedApprovalAttempt({
    tenantId,
    attempt: {
      attemptId,
      createdAt: startedAt,
...
async function handleTenantOnboardingPage(req, res, tenantId, url) {
  ...
  const html = [
    ...
    "<div class=\"muted\">Seed the first hosted approval, then run an end-to-end paid marketplace flow (register, fund, RFQ, bid, accept, settle, verify), persist attempts, and replay prior runs.</div>",
    "<div class=\"row\" style=\"margin-top:8px\">",
    "<div class=\"field small\"><div class=\"muted\">Launch host</div><select id=\"seedHostedApprovalTrack\"><option value=\"claude\">Claude MCP</option><option value=\"openclaw\">OpenClaw</option><option value=\"codex\">Codex / CLI / API</option></select></div>",
    "<button class=\"btn success\" id=\"seedHostedApprovalBtn\">Create hosted approval</button>",
    "<a class=\"btn ghost\" id=\"seedHostedApprovalOpen\" target=\"_blank\" rel=\"noreferrer\">Open approval</a>",
    "</div>",
    "<div id=\"seedHostedApprovalStatus\" class=\"status\" style=\"margin-top:8px\">Hosted approval not created yet.</div>",
    "<pre id=\"seedHostedApprovalOutput\" class=\"mono\" style=\"white-space:pre-wrap;background:#0b1020;color:#f8fafc;border-radius:12px;padding:10px;max-height:220px;overflow:auto;margin-top:8px\">{}</pre>",
    "<div class=\"row\" style=\"margin-top:8px\">",
    ...
    "<script>",
    `const tenantId = ${JSON.stringify(tenantId)};`,
    "const state = { templates: [], selectedTemplate: null, renderedTemplate: null, token: null, onboardingMetrics: null, runtimeBootstrap: null, runtimeSmoke: null, seededApproval: null, firstPaidCall: null, firstPaidHistory: null, runtimeConformance: null };",
    "function setText(id, text){ const el=document.getElementById(id); if(el) el.textContent=String(text||''); }",
    "function setHtml(id, html){ const el=document.getElementById(id); if(el) el.innerHTML=String(html||''); }",
    ...
    "function renderSeededApproval(out){ const status=document.getElementById('seedHostedApprovalStatus'); const pre=document.getElementById('seedHostedApprovalOutput'); const open=document.getElementById('seedHostedApprovalOpen'); if(!status||!pre||!open) return; if(!out||typeof out!=='object'){ status.className='status'; status.textContent='Hosted approval not created yet.'; pre.textContent='{}'; open.removeAttribute('href'); open.setAttribute('aria-disabled','true'); return; } const requestId=String(out&&out.approvalRequest&&out.approvalRequest.requestId?out.approvalRequest.requestId:'n/a'); const hostTrack=String(out&&out.hostTrack?out.hostTrack:'unknown'); const approvalUrl=String(out&&out.approvalUrl?out.approvalUrl:'').trim(); status.className='status good'; status.textContent=`Hosted approval ${requestId} is live for ${hostTrack}.`; pre.textContent=JSON.stringify(out, null, 2); if(approvalUrl){ open.href=approvalUrl; open.removeAttribute('aria-disabled'); } else { open.removeAttribute('href'); open.setAttribute('aria-disabled','true'); } }",
    "async function runSeedHostedApproval(){ const status=document.getElementById('seedHostedApprovalStatus'); const select=document.getElementById('seedHostedApprovalTrack'); if(!status||!select) return; const hostTrack=String(select.value||'claude').trim().toLowerCase(); status.className='status'; status.textContent=`Creating hosted approval for ${hostTrack}…`; try{ const out=await postJson(`/v1/tenants/${encodeURIComponent(tenantId)}/onboarding/seed-hosted-approval`, { hostTrack }); state.seededApproval=out; renderSeededApproval(out); trackOnboardingEvent('hosted_approval_seeded', { hostTrack, requestId: out&&out.approvalRequest&&out.approvalRequest.requestId?out.approvalRequest.requestId:null }); await refreshChecklist(); } catch(e){ status.className='status bad'; status.textContent='Hosted approval failed: '+e.message; } }",
    ...
    "document.getElementById('seedHostedApprovalBtn').addEventListener('click', ()=>runSeedHostedApproval());",
...
export async function magicLinkHandler(req, res) {
  ...
  try {
    ...
    const tenantSeedHostedApprovalHistoryMatch = /^\/v1\/tenants\/([a-zA-Z0-9_-]{1,64})\/onboarding\/seed-hosted-approval\/history$/.exec(pathname);
    if (tenantSeedHostedApprovalHistoryMatch) {
      const tenantId = tenantSeedHostedApprovalHistoryMatch[1];
      if (method === "GET") return await handleTenantHostedApprovalHistory(req, res, tenantId);
      return sendText(res, 405, "method not allowed\n");
    }
