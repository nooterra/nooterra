...
export function buildNooterraPayKeysetV1({ activeKey, fallbackKeys = [], refreshedAt = new Date().toISOString() } = {}) {
...
export function publicKeyPemFromNooterraKeysetEntry(entry) {
...
export function keyMapFromNooterraKeyset(keyset) {
  ...
  for (const row of rows) {
    ...
    const publicKeyPem = publicKeyPemFromNooterraKeysetEntry(row);
