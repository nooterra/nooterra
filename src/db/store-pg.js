...
import { createDeliveryRepository } from "./repositories/deliveries.js";
import { createAuthKeysRepository } from "./repositories/auth-keys.js";
import { createAgentsRepository } from "./repositories/agents.js";
import { createSessionsRepository } from "./repositories/sessions.js";
import { createGovernanceRepository } from "./repositories/governance.js";
...
export async function createPgStore({ databaseUrl, schema = "public", dropSchemaOnClose = false, migrateOnStartup = true } = {}) {
  ...
  const deliveryRepo = createDeliveryRepository({
    pool,
    getConfig: (tid) => (typeof store.getConfig === "function" ? store.getConfig(tid) : store.config),
    withTx
  });

  const authKeysRepo = createAuthKeysRepository({
    pool,
    authKeys: store.authKeys,
    signerKeys: store.signerKeys,
    publicKeyByKeyId: store.publicKeyByKeyId
  });
  ...
  const agentsRepo = createAgentsRepository({
    ...
    agentPassports: store.agentPassports,
    agentWallets: store.agentWallets,
    agentRuns: store.agentRuns,
    agentRunEvents: store.agentRunEvents,
    agentRunSettlements: store.agentRunSettlements
  });

  const sessionsRepo = createSessionsRepository({
    pool,
    sessions: store.sessions,
    sessionEvents: store.sessionEvents
  });

  const governanceRepo = createGovernanceRepository({
    pool,
    approvalRequests: store.approvalRequests,
    approvalDecisions: store.approvalDecisions,
    approvalStandingPolicies: store.approvalStandingPolicies,
    approvalContinuations: store.approvalContinuations
  });

  const tasksRepo = createTasksRepository({
    pool,
    taskQuotes: store.taskQuotes,
    taskOffers: store.taskOffers,
    taskAcceptances: store.taskAcceptances,
    capabilityAttestations: store.capabilityAttestations
  });

  const marketplaceRepo = createMarketplaceRepository({
    pool,
    withTx,
    persistSnapshotAggregate,
    insertOpsAuditRow,
    marketplaceProviderPublications: store.marketplaceProviderPublications,
    marketplaceCapabilityListings: store.marketplaceCapabilityListings
  });

  const financeRepo = createFinanceRepository({
    pool,
    withTx,
    persistSnapshotAggregate,
    ...
    getConfig: (tid) => (typeof store.getConfig === "function" ? store.getConfig(tid) : store.config),
    moneyRailOperations: store.moneyRailOperations,
    moneyRailProviderEvents: store.moneyRailProviderEvents,
    billableUsageEvents: store.billableUsageEvents,
    financeReconciliationTriages: store.financeReconciliationTriages
  });
  ...
  store.getAuthKey = authKeysRepo.getAuthKey;

  store.listAuthKeys = authKeysRepo.listAuthKeys;

  store.putAuthKey = authKeysRepo.putAuthKey;

  store.touchAuthKey = authKeysRepo.touchAuthKey;

  store.setAuthKeyStatus = authKeysRepo.setAuthKeyStatus;

  store.rotateAuthKey = authKeysRepo.rotateAuthKey;
  ...
  store.getSignerKey = authKeysRepo.getSignerKey;

  store.listSignerKeys = authKeysRepo.listSignerKeys;
  ...
  store.listAgentCardsPublic = agentsRepo.listAgentCardsPublic;
  ...
  store.getAgentRunEvents = agentsRepo.getAgentRunEvents;
  store.getAgentRunSettlement = agentsRepo.getAgentRunSettlement;
  store.listAgentRunSettlements = agentsRepo.listAgentRunSettlements;

  // --- Sessions repository delegation ---
  store.getSession = sessionsRepo.getSession;
  store.listSessions = sessionsRepo.listSessions;
  store.getSessionEvents = sessionsRepo.getSessionEvents;

  // --- Governance repository delegation ---
  Object.assign(store, governanceRepo);

  // --- Tasks repository delegation ---
  Object.assign(store, tasksRepo);

  // --- Marketplace repository delegation ---
  Object.assign(store, marketplaceRepo);

  // --- Finance repository delegation ---
  Object.assign(store, financeRepo);

  store.putSignerKey = authKeysRepo.putSignerKey;

  store.setSignerKeyStatus = authKeysRepo.setSignerKeyStatus;
  ...
  store.lookupCorrelation = deliveryRepo.lookupCorrelation;

  store.upsertCorrelation = deliveryRepo.upsertCorrelation;

  store.listCorrelations = deliveryRepo.listCorrelations;

  store.getIngestRecord = deliveryRepo.getIngestRecord;

  store.listIngestRecords = deliveryRepo.listIngestRecords;
  ...
  store.createDelivery = deliveryRepo.createDelivery;

  store.listDeliveries = deliveryRepo.listDeliveries;
  ...
  store.claimDueDeliveries = deliveryRepo.claimDueDeliveries;

  store.updateDeliveryAttempt = deliveryRepo.updateDeliveryAttempt;

  store.requeueDelivery = deliveryRepo.requeueDelivery;

  store.ackDelivery = deliveryRepo.ackDelivery;
