...
test("api-sdk: delegation grant methods call expected endpoints", async () => {
  ...
  assert.equal(calls[0].url, "https://api.settld.local/delegation-grants");
  ...
  assert.equal(calls[1].url, "https://api.settld.local/delegation-grants");
  ...
  assert.equal(
    calls[2].url,
    "https://api.settld.local/delegation-grants?delegateeAgentId=agt_worker&includeRevoked=false&limit=20&offset=0"
  );
  assert.equal(calls[2].init?.method, "GET");

  await client.getDelegationGrant(grantId);
  assert.equal(calls[3].url, `https://api.settld.local/delegation-grants/${grantId}`);
  assert.equal(calls[3].init?.method, "GET");

  await client.revokeDelegationGrant(grantId, { reasonCode: "MANUAL_REVOKE" });
  assert.equal(calls[4].url, `https://api.settld.local/delegation-grants/${grantId}/revoke`);
...
test("api-sdk: work-order methods call expected endpoints", async () => {
  ...
  assert.equal(calls[0].url, "https://api.settld.local/work-orders");
  assert.equal(calls[0].init?.method, "POST");

  await client.listWorkOrders({ workOrderId, status: "created", limit: 20, offset: 0 });
  assert.equal(calls[1].url, "https://api.settld.local/work-orders?workOrderId=workord_sdk_1&status=created&limit=20&offset=0");
  assert.equal(calls[1].init?.method, "GET");

  await client.getWorkOrder(workOrderId);
  assert.equal(calls[2].url, `https://api.settld.local/work-orders/${workOrderId}`);
  assert.equal(calls[2].init?.method, "GET");

  await client.acceptWorkOrder(workOrderId, { acceptedByAgentId: "agt_worker" });
  assert.equal(calls[3].url, `https://api.settld.local/work-orders/${workOrderId}/accept`);
  assert.equal(calls[3].init?.method, "POST");

  await client.progressWorkOrder(workOrderId, { eventType: "progress", message: "working" });
  assert.equal(calls[4].url, `https://api.settld.local/work-orders/${workOrderId}/progress`);
  assert.equal(calls[4].init?.method, "POST");

  await client.topUpWorkOrder(workOrderId, { topUpId: "topup_1", amountCents: 100 });
  assert.equal(calls[5].url, `https://api.settld.local/work-orders/${workOrderId}/topup`);
  assert.equal(calls[5].init?.method, "POST");

  await client.completeWorkOrder(workOrderId, { receiptId, status: "success" });
  assert.equal(calls[6].url, `https://api.settld.local/work-orders/${workOrderId}/complete`);
  ...
  assert.equal(calls[7].url, `https://api.settld.local/work-orders/${workOrderId}/settle`);
  ...
  assert.equal(
    calls[8].url,
    "https://api.settld.local/work-orders/receipts?workOrderId=workord_sdk_1&principalAgentId=agt_principal&status=success&limit=10&offset=0"
  );
  assert.equal(calls[8].init?.method, "GET");

  await client.getWorkOrderReceipt(receiptId);
  assert.equal(calls[9].url, `https://api.settld.local/work-orders/receipts/${receiptId}`);
...
test("api-sdk: capability attestation methods call expected endpoints", async () => {
  ...
  assert.equal(calls[0].url, "https://api.settld.local/capability-attestations");
  assert.equal(calls[0].init?.method, "POST");
  ...
  assert.equal(
    calls[1].url,
    "https://api.settld.local/capability-attestations?subjectAgentId=agt_worker&status=active&includeInvalid=false&limit=25&offset=0"
  );
  assert.equal(calls[1].init?.method, "GET");

  await client.getCapabilityAttestation(attestationId);
  assert.equal(calls[2].url, `https://api.settld.local/capability-attestations/${attestationId}`);
  assert.equal(calls[2].init?.method, "GET");

  await client.revokeCapabilityAttestation(attestationId, { reasonCode: "MANUAL_REVOKE" });
  assert.equal(calls[3].url, `https://api.settld.local/capability-attestations/${attestationId}/revoke`);
  assert.equal(calls[3].init?.method, "POST");
});
