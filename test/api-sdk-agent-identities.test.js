...
test("api-sdk: agent identity methods call expected endpoints", async () => {
  ...
  assert.equal(calls[0].url, "https://api.settld.local/agents/register");
  assert.equal(calls[0].init?.method, "POST");

  await client.listAgents({ status: "active", limit: 10, offset: 0 });
  assert.equal(calls[1].url, "https://api.settld.local/agents?status=active&limit=10&offset=0");
  assert.equal(calls[1].init?.method, "GET");

  await client.getAgent("agt_demo");
  assert.equal(calls[2].url, "https://api.settld.local/agents/agt_demo");
