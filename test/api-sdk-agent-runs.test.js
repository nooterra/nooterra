...
test("api-sdk: agent run methods call expected endpoints", async () => {
  ...
  assert.equal(calls[0].url, "https://api.settld.local/agents/agt_demo/wallet");
  assert.equal(calls[0].init?.method, "GET");

  await client.creditAgentWallet("agt_demo", { amountCents: 1000, currency: "USD" });
  assert.equal(calls[1].url, "https://api.settld.local/agents/agt_demo/wallet/credit");
  assert.equal(calls[1].init?.method, "POST");

  await client.createAgentRun("agt_demo", { runId: "run_demo_1" });
  assert.equal(calls[2].url, "https://api.settld.local/agents/agt_demo/runs");
  ...
  assert.equal(
    calls[3].url,
    "https://api.settld.local/agents?status=active&capability=translate&minTrustScore=90&includeReputation=true&reputationVersion=v2&reputationWindow=30d&limit=5&offset=0"
  );
  assert.equal(calls[3].init?.method, "GET");

  await client.getAgentReputation("agt_demo", { reputationVersion: "v2", reputationWindow: "7d" });
  assert.equal(calls[4].url, "https://api.settld.local/agents/agt_demo/reputation?reputationVersion=v2&reputationWindow=7d");
  ...
  assert.equal(
    calls[5].url,
    "https://api.settld.local/marketplace/agents/search?capability=translate&minTrustScore=80&riskTier=low&includeReputation=true&reputationVersion=v2&reputationWindow=30d&scoreStrategy=recent_bias&limit=10&offset=0"
  );
  assert.equal(calls[5].init?.method, "GET");

  await client.listAgentRuns("agt_demo", { status: "completed", limit: 10, offset: 0 });
  assert.equal(calls[6].url, "https://api.settld.local/agents/agt_demo/runs?status=completed&limit=10&offset=0");
  assert.equal(calls[6].init?.method, "GET");

  await client.getAgentRun("agt_demo", "run_demo_1");
  assert.equal(calls[7].url, "https://api.settld.local/agents/agt_demo/runs/run_demo_1");
  assert.equal(calls[7].init?.method, "GET");

  await client.listAgentRunEvents("agt_demo", "run_demo_1");
  assert.equal(calls[8].url, "https://api.settld.local/agents/agt_demo/runs/run_demo_1/events");
  ...
  assert.equal(calls[9].url, "https://api.settld.local/agents/agt_demo/runs/run_demo_1/events");
  assert.equal(calls[9].init?.method, "POST");

  await client.getRunVerification("run_demo_1");
  assert.equal(calls[10].url, "https://api.settld.local/runs/run_demo_1/verification");
  assert.equal(calls[10].init?.method, "GET");

  await client.getRunSettlement("run_demo_1");
  assert.equal(calls[11].url, "https://api.settld.local/runs/run_demo_1/settlement");
  ...
  assert.equal(calls[12].url, "https://api.settld.local/runs/run_demo_1/dispute/open");
  assert.equal(calls[12].init?.method, "POST");

  await client.closeRunDispute("run_demo_1", { disputeId: "dsp_1", resolutionOutcome: "accepted" });
  assert.equal(calls[13].url, "https://api.settld.local/runs/run_demo_1/dispute/close");
  ...
  assert.equal(calls[14].url, "https://api.settld.local/runs/run_demo_1/dispute/evidence");
