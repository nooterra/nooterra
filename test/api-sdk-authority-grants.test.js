...
test("api-sdk: authority grant methods call expected endpoints", async () => {
  ...
  assert.equal(calls[0].url, "https://api.settld.local/authority-grants");
  assert.equal(calls[0].init?.method, "POST");

  await client.listAuthorityGrants({ granteeAgentId: "agt_worker", includeRevoked: false, limit: 20, offset: 0 });
  assert.equal(calls[1].url, "https://api.settld.local/authority-grants?granteeAgentId=agt_worker&includeRevoked=false&limit=20&offset=0");
  assert.equal(calls[1].init?.method, "GET");

  await client.getAuthorityGrant(grantId);
  assert.equal(calls[2].url, `https://api.settld.local/authority-grants/${grantId}`);
  assert.equal(calls[2].init?.method, "GET");

  await client.revokeAuthorityGrant(grantId, { revocationReasonCode: "MANUAL_REVOKE" });
  assert.equal(calls[3].url, `https://api.settld.local/authority-grants/${grantId}/revoke`);
