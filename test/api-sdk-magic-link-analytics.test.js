...
test("api-sdk: tenant analytics + trust graph methods call expected endpoints", async () => {
  ...
  const client = new NooterraClient({
    baseUrl: "https://magic-link.nooterra.local",
    tenantId: "tenant_sdk",
  ...
  assert.equal(
    calls[0].url,
    "https://magic-link.nooterra.local/v1/tenants/tenant_demo/analytics?month=2026-02&bucket=week&limit=10"
  );
  ...
  assert.equal(
    calls[1].url,
    "https://magic-link.nooterra.local/v1/tenants/tenant_demo/trust-graph?month=2026-02&minRuns=3&maxEdges=150"
  );
  assert.equal(calls[1].init?.method, "GET");

  await client.listTenantTrustGraphSnapshots("tenant_demo", { limit: 25 });
  assert.equal(calls[2].url, "https://magic-link.nooterra.local/v1/tenants/tenant_demo/trust-graph/snapshots?limit=25");
  assert.equal(calls[2].init?.method, "GET");

  await client.createTenantTrustGraphSnapshot("tenant_demo", { month: "2026-02", minRuns: 5, maxEdges: 200 });
  assert.equal(calls[3].url, "https://magic-link.nooterra.local/v1/tenants/tenant_demo/trust-graph/snapshots");
  ...
  assert.equal(
    calls[4].url,
    "https://magic-link.nooterra.local/v1/tenants/tenant_demo/trust-graph/diff?baseMonth=2026-01&compareMonth=2026-02&limit=50&minRuns=2&maxEdges=120&includeUnchanged=true"
  );
