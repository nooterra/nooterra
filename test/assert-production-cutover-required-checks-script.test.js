...
test("production cutover required checks: fails closed when lineage check is missing", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "settld-prod-required-checks-missing-"));
  t.after(async () => {
    await fs.rm(root, { recursive: true, force: true });
  });
  ...
  await writeJson(inputPath, {
    schemaVersion: "ProductionCutoverGateReport.v1",
    verdict: { ok: true },
    checks: [{ id: "settld_verified_collaboration", status: "passed" }]
  });
...
test("production cutover required checks: appends markdown summary when GITHUB_STEP_SUMMARY is set", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "settld-prod-required-checks-summary-"));
  t.after(async () => {
    await fs.rm(root, { recursive: true, force: true });
  });
