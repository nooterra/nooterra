...
test("mcp paid weather tool: retries x402 via autopay and returns weather payload", async () => {
  const requests = [];
    ...
    if (req.method === "GET" && url.pathname === "/weather/current") {
      ...
      if (!gateId) {
        res.writeHead(402, {
          ...
          "x-payment-required": "amountCents=500; currency=USD; address=mock:weather; network=mocknet",
          "x-nooterra-gate-id": "gate_weather_paid_1"
        });
      ...
      res.writeHead(200, {
        ...
        "x-nooterra-policy-hash": "b".repeat(64),
        "x-nooterra-policy-version": "1",
        "x-nooterra-decision-id": "dec_mcp_paid_weather_1",
  ...
  const called = await rpc("tools/call", {
    ...
    arguments: { city: "Chicago", unit: "f" }
  });
  assert.equal(called.result?.isError, false);
  const payload = JSON.parse(called.result?.content?.[0]?.text ?? "{}");
  assert.equal(payload.tool, "nooterra.weather_current_paid");
  assert.equal(payload.result?.ok, true);
  assert.equal(payload.result?.response?.provider, "weather-mock");
  ...
  assert.equal(requests[1]?.gateId, "gate_weather_paid_1");
