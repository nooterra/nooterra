...
import { FINANCE_RECONCILE_ADVISORY_LOCK_KEY } from "../src/core/maintenance-locks.js";
import { request } from "./api-test-harness.js";

const databaseUrl = process.env.DATABASE_URL ?? null;

(databaseUrl ? test : test.skip)("pg: finance reconcile maintenance endpoint returns 409 when run lock is held", async () => {
  ...
  try {
    ...
    try {
      await client.query("SELECT pg_advisory_lock(hashtext($1))", [FINANCE_RECONCILE_ADVISORY_LOCK_KEY]);
      ...
      try {
        await client.query("SELECT pg_advisory_unlock(hashtext($1))", [FINANCE_RECONCILE_ADVISORY_LOCK_KEY]);
      } catch {}
      client.release();
    }
    ...
    assert.equal(audit[0].action, "MAINTENANCE_FINANCE_RECONCILE_RUN");
    assert.equal(audit[0].details?.path, "/ops/maintenance/finance-reconcile/run");
