...
test("public website route smoke: reports success when all routes return branded HTML", async () => {
  const seenUrls = [];
  const allNeedlesBody = "<!DOCTYPE html><html><body>Give agents wallets, not unchecked permissions. /signup?source=home for every consequential agent action. /signup?source=product Free to build. /signup?source=pricing Add an Action Wallet in minutes. /signup?source=developers Connect where agents already run. /signup?source=integrations One wallet for every consequential AI action. Know exactly what you are approving. Every action should end in a readable record. If something goes wrong, there has to be a path back. Create the account. Create the workspace before the agent gets to act. Return to the same workspace, approvals, and receipts. Create the account first. Documentation with the website as the index Start with one real action, not a giant setup ritual. Understand the control plane before you trust it. should all resolve into the same approval, receipt, and dispute surfaces. The API should feel like one product, not a bag of endpoints. Security should explain the boundaries, not just claim them. Operator pages should lead to runbooks, not leave you guessing. Claude should reach its first approval without leaving people guessing. OpenClaw should prove host-native parity, not invent a second product. The shortest engineering path should still leave a real approval and receipt. Set up the repo once, then get back to proving the live loop. Every supported host should land on the same approval Design partners should get one disciplined onboarding pack A production claim should map to a concrete release bar When something goes wrong, the support path should already exist Live route health should be visible. Support should route users into the right trust surface fast Security for Nooterra means bounded authority The public site should explain the data boundary before people enter the product. host-first Action Wallet The approval window closed before the action could continue This authority was revoked before execution could continue The action completed, but the proof did not verify This host is outside the launch support envelope</body></html>";
  ...
  assert.equal(report.checks.length, 38);
