...
test("create-nooterra-paid-tool scaffold writes runnable template files", async () => {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), "nooterra-scaffold-paid-tool-"));
  const outDir = path.join(tmpRoot, "paid-tool");
  const exec = await runNode({
    cwd: REPO_ROOT,
    args: ["scripts/scaffold/create-nooterra-paid-tool.mjs", outDir, "--provider-id", "prov_test_scaffold_1"]
  });
  ...
  assert.equal(pkg.dependencies?.["@nooterra/provider-kit"], "latest");

  const serverSrc = await readFile(path.join(outDir, "server.mjs"), "utf8");
  assert.match(serverSrc, /createNooterraPaidNodeHttpHandler/);
  const readme = await readFile(path.join(outDir, "README.md"), "utf8");
  assert.match(readme, /NooterraPay/);
});

test("create-nooterra-paid-tool package bin scaffolds template", async () => {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), "nooterra-scaffold-paid-tool-bin-"));
  const outDir = path.join(tmpRoot, "paid-tool-bin");
  const exec = await runNode({
    cwd: REPO_ROOT,
    args: ["packages/create-nooterra-paid-tool/bin/create-nooterra-paid-tool.js", outDir, "--provider-id", "prov_test_pkg_bin_1"]
  });
  assert.equal(exec.code, 0, `stderr=${exec.stderr}`);
  assert.match(exec.stdout, /created=/);
...
test("create-nooterra-paid-tool --from-http writes bridge manifest + mcp bridge", async () => {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), "nooterra-scaffold-paid-tool-http-"));
  const outDir = path.join(tmpRoot, "paid-tool-http");
  const exec = await runNode({
    cwd: REPO_ROOT,
    args: ["scripts/scaffold/create-nooterra-paid-tool.mjs", outDir, "--provider-id", "prov_http_1", "--from-http", "https://api.example.com"]
  });
  assert.equal(exec.code, 0, `stderr=${exec.stderr}`);
  ...
  const pkg = JSON.parse(await readFile(path.join(outDir, "package.json"), "utf8"));
  assert.equal(pkg.dependencies?.["nooterra-api-sdk"], "latest");
});

test("create-nooterra-paid-tool --from-openapi generates bridge tools from spec", async () => {
  const tmpRoot = await mkdtemp(path.join(os.tmpdir(), "nooterra-scaffold-paid-tool-openapi-"));
  ...
  const exec = await runNode({
    cwd: REPO_ROOT,
    args: ["scripts/scaffold/create-nooterra-paid-tool.mjs", outDir, "--provider-id", "prov_openapi_1", "--from-openapi", openApiPath]
  });
  assert.equal(exec.code, 0, `stderr=${exec.stderr}`);
