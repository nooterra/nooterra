...
function parseExplain(stderr) {
  ...
  for (const line of String(stderr ?? "").split("\n")) {
    ...
    if (!trimmed || trimmed === "nooterra-verify explain v1") continue;
...
test("nooterra-verify --explain prints deterministic diagnostics to stderr without breaking JSON stdout", async () => {
  ...
  assert.ok(res.stderr.includes("nooterra-verify explain v1"));
...
test("nooterra-verify --explain includes the primary strict failure reason for a fixture", async () => {
  const trustPath = path.resolve(process.cwd(), "test", "fixtures", "bundles", "v1", "trust.json");
  const trust = JSON.parse(await fs.readFile(trustPath, "utf8"));
  const env = { NOOTERRA_TRUSTED_GOVERNANCE_ROOT_KEYS_JSON: JSON.stringify(trust.governanceRoots ?? {}) };
